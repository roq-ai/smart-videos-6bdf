...
import { getClicks } from 'apiSdk/clicks';
import { ClickInterface } from 'interfaces/click';
import { getActions } from 'apiSdk/actions';
...
import { ClickListPage as ShowCaseList } from 'pages/clicks';

function DashboardPage() {
  const { session } = useSession();

  const clickStat = useSWR<PaginatedInterface<ClickInterface>>(
    () => `/clicks`,
    () => getClicks({ limit: 1, offset: 0 }),
  );

  const actionStat = useSWR<PaginatedInterface<ActionInterface>>(
    () => `/actions`,
    () => getActions({ limit: 1, offset: 0 }),
  );

  const videoPlayerStat = useSWR<PaginatedInterface<VideoPlayerInterface>>(
    () => `/video-players`,
    () => getVideoPlayers({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Clicks',
      result: clickStat?.data?.totalCount,
      isLoading: clickStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Actions',
      result: actionStat?.data?.totalCount,
      isLoading: actionStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Video Players',
      result: videoPlayerStat?.data?.totalCount,
      isLoading: videoPlayerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`clicks`} passHref legacyBehavior>
                  <Button
